...
class LogInActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //signOut() ---> 버튼을 눌렀을 때 함수호출 되도록 나중에 수정
    }
    ...
    private fun firebaseAuthWithGoogle(acct: GoogleSignInAccount) {
        Log.d("SplashActivity", "firebaseAuthWithGoogle:" + acct.id!!)
        ...
        firebaseAuth.signInWithCredential(credential)
            .addOnCompleteListener(this) { task ->
                if (task.isSuccessful) {
                    Log.w("SplashActivity", "firebaseAuthWithGoogle 성공", task.exception)
                    toMainActivity(firebaseAuth?.currentUser)
                } else {
                    Log.w("SplashActivity", "firebaseAuthWithGoogle 실패", task.exception)
